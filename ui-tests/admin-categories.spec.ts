...
test.describe("Admin category management", () => {
  ...
  test("Category Creation with Special Characters: verify handling of valid special characters", async ({
    ...
    // Verify in list
    await expect(page.locator("tbody")).toContainText(categoryWithSpecialChars);
  });
  ...
  // Li Jiakai, A0252287Y
  test("[bug] Category Update: editing to duplicate category", async ({
    page,
  }) => {
    ...
    // Verify update success message
    await expect(page.getByText(`Category Clothing is updated`)).toBeVisible();
