...
describe("Product Controller Additional Integration Tests", () => {
  ...
  describe("getProductController", () => {
    it("getProductController + productModel populate/select/limit/sort integration: returns 200 with seeded products, populated category, and no photo field", async () => {
      ...
      expect(payload.products.length).toBeLessThanOrEqual(12);
      expect(payload.countTotal).toBe(payload.products.length);
    ...
    it("getProductController + productModel limit integration: returns at most 12 products when more exist", async () => {
      for (let i = 0; i < 15; i += 1) {
        await createProduct({ name: `Bulk Product ${i}` });
      }
      ...
      expect(res.status).toHaveBeenCalledWith(200);
      expect(payload.products.length).toBe(12);
